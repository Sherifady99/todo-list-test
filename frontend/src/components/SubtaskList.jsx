...
export default function SubtaskList({ taskId }) {
  ...
  if (loading) {
    return <div className="mt-3 text-xs text-slate-400 pl-1">Loading subtasks…</div>;
  }
      ...
      {showInput ? (
        <form onSubmit={handleAdd} className="mt-2 flex items-center gap-2">
          ...
          <button
            ...
            className="px-2.5 py-1 text-sm font-medium bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            Add
          </button>
          <button
            ...
            className="px-2 py-1 text-sm text-slate-500 hover:text-slate-700 rounded-lg hover:bg-slate-100 transition-colors"
          >
            Cancel
          </button>
        </form>
